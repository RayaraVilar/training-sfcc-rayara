...
/**
 * Calcula a porcentagem de desconto entre o preço original e o preço promocional.
 *
 * @param {number} priceOriginal - O preço original do produto.
 * @param {number} pricePromo - O preço promocional do produto.
 * @returns {number} - A porcentagem de desconto arredondada.
 */
function calculateDiscountPercentage(priceOriginal, pricePromo) {
    if (priceOriginal <= 0 || pricePromo <= 0 || pricePromo >= priceOriginal) {
        return 0;
    }

    var discountPercentage = ((priceOriginal - pricePromo) / priceOriginal) * 100;
    return Math.round(discountPercentage); // Arredonda para valor inteiro
}

/**
 * Obtém a mensagem de desconto, que é apenas a porcentagem de desconto.
 *
 * @param {number} priceOriginal - O preço original do produto.
 * @param {number} pricePromo - O preço promocional do produto.
 * @returns {number} - A porcentagem de desconto.
 */
function getDiscountMessage(priceOriginal, pricePromo) {
    var discountPercentage = calculateDiscountPercentage(priceOriginal, pricePromo);
    return discountPercentage; // Retorna apenas o número X
}
